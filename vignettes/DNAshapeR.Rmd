...
* how to visualize DNA shapre prediction
...
The result of the feature encoding is a matrix consisted of concatenated feature vectors from given sequences. The matrix can serve as an input for statistical machine learning methods. The application of modeling by multiple linear regression (MLR) upon high-throughput SELEX-seq testing data is demonstrated below.

First, combine the binding affinity values with respect encoded feature vectors, forming a dataframe structure.
...
Then, apply a macine learning package (can be any tools). Here caret package (see <http://caret.r-forge.r-project.org/> for more information) is employed.
